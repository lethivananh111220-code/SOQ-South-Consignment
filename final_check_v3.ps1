...
if ($file) {
    ...
    $tempPath = "d:\DHF\QLKV_WM\web_app\temp_final_v3.xlsx"
    ...
    if ($wsEntry) {
        ...
        # Find column for 46113 (April 1st, 2026)
        $targetColIdx = -1
        for ($i = 0; $i -lt $headers.Count; $i++) {
            $h = $headers[$i]
            if ($h -eq "46113") {
                $targetColIdx = $i
                Write-Output "Found target column: '$h' (2026-04-01) at index $i"
            }
        }
        
        if ($targetColIdx -ne -1) {
            Write-Output "Stores scheduled for delivery on 2026-04-01 (Tomorrow):"
            ...
            Write-Output "Header 46113 not found."
        }
    }
    $zip.Dispose()
